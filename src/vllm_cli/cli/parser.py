...
def parse_model_length(value: str):
    """
    Parse model length value that supports formats like:
    - "1M" -> 1000000
    - "100K" -> 100000
    - "1K" -> 1000
    - Plain integers
    ...
    Args:
        value: String value to parse (e.g., "1M", "100K", "1000")

    Returns:
        Integer value or None if empty/None
    ...
    # Pattern to match formats like "1M", "100K", "1K"
    pattern = r"^(\d+)([MKmk])?$"
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid model length: {value}. Use formats like 1M, 100K, or plain integers."
        )
...
def _add_serve_parser(subparsers) -> None:
    ...
    serve_parser.add_argument(
        "--max-model-len",
        type=parse_model_length,
        help="Maximum model sequence length (supports 1M, 100K format)",
    )
...
def _add_recipes_parser(subparsers) -> None:
    """Add the 'recipes' subcommand parser."""
    recipes_parser = subparsers.add_parser(
        "recipes",
        help="Manage official vLLM recipes",
        description="Sync and import official vLLM recipe configurations as profiles",
    )

    recipes_parser.add_argument(
        "--list",
        action="store_true",
        help="List available recipes from vLLM official repository",
    )

    recipes_parser.add_argument(
        "--sync",
        action="store_true",
        help="Sync and import selected recipes as profiles",
    )

    recipes_parser.add_argument(
        "--import",
        dest="import_recipe",
        help="Import a specific recipe by ID",
    )

    recipes_parser.add_argument(
        "--sync-args",
        action="store_true",
        help="Sync CLI arguments from vLLM GitHub source",
    )

    recipes_parser.add_argument(
        "--sync-parsers",
        action="store_true",
        help="Sync reasoning_parser and tool_call_parser choices from vLLM GitHub source",
    )

    recipes_parser.add_argument(
        "--apply-args",
        action="store_true",
        help="Apply synced CLI arguments to local schema (use with --sync-args)",
    )
