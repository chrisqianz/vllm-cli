...
def handle_recipes(args: argparse.Namespace) -> bool:
    ...
    if args.list:
        try:
            ...
            table = Table(title="Available vLLM Recipes")
        ...
        try:
            ...
            if profile:
                console.print(f"[green]Recipe imported successfully![/green]")
                console.print(f"Profile: {profile.get('name')}")
                return True
            else:
                console.print(
                    f"[red]Failed to import recipe: {args.import_recipe}[/red]"
                )
                return False
        except Exception as e:
            console.print(f"[red]Error importing recipe: {e}[/red]")
            return False
