    ...
    if arg_type == "boolean":
        ...
        if arg_name == "quantization":
            choice_list = [
                ...
                "modelopt_fp4 - ModelOpt NVFP4 (FP4 weight quantization)",
                "modelopt_mxfp8 - ModelOpt MXFP8 (Mixed-precision FP8)",
                "Skip (use default)",
            ]
        ...
        if arg_name == "max_model_len":
            ...
            if response:
                ...
                if parsed is not None:
                    ...
                    console.print("[red]Invalid format, skipping max_model_len[/red]")
        ...
        if arg_name == "chat_template":
            ...
            # Offer options
            options = ["Enter file path manually", "Browse for .jinja files", "Skip"]
            if current_value:
                options.insert(0, f"Keep current: {current_value}")
            ...
            if choice == "Skip" or not choice:
                pass  # Don't set chat_template
            elif choice and choice.startswith("Keep current:"):
                # Keep existing value
                pass
            elif choice == "Browse for .jinja files":
                ...
                if jinja_files:
                    ...
                    jinja_files.append("← Enter path manually")

                    selected_file = unified_prompt(
                        "select_jinja",
                    ...
                    if selected_file and selected_file != "← Enter path manually":
                        config[arg_name] = selected_file
                        console.print(
                            f"[green]Chat template set to: {selected_file}[/green]"
                        )
                    elif selected_file == "← Enter path manually":
                        response = input("Enter chat template file path: ").strip()
                        if response:
                            if os.path.exists(response):
                                config[arg_name] = response
                                console.print(
                                    f"[green]Chat template set to: {response}[/green]"
                                )
                            else:
                                console.print(
                                    f"[yellow]Warning: File not found: {response}[/yellow]"
                                )
                    ...
                    if response:
                        config[arg_name] = response
            elif choice == "Enter file path manually":
                if current_value:
                    prompt_text = f"Enter file path (current: {current_value}) or press Enter to skip: "
                else:
                    prompt_text = "Enter file path or press Enter to skip: "
            ...
            if sensitive:
                ...
                if current_value:
                    prompt_text = f"Enter value (current: {current_value}) or press Enter to skip: "
                else:
...
def configure_environment_variables(
    ...
    env_presets = {
        ...
        "MoE Model Optimization": {
            "VLLM_USE_FLASHINFER_MXFP4_MOE": "Enable FlashInfer MXFP4 for MoE (recommended for GPT-OSS, usually 1)",
