...
class ProfileManager:
    ...
    def rename_user_profile(self, old_name: str, new_name: str) -> bool:
        """Rename a user profile."""
        if old_name not in self.user_profiles:
            logger.error(f"Profile '{old_name}' not found")
            return False
        if new_name in self.user_profiles:
            logger.error(f"Profile '{new_name}' already exists")
            return False
        self.user_profiles[new_name] = self.user_profiles.pop(old_name)
        return self._save_user_profiles()
