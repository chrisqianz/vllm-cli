...
from .i18n import I18nManager, LanguageSelector
...
def main() -> NoReturn:
    ...
    try:
        ...
        # Show language selector if needed (first launch)
        language_selector = LanguageSelector(i18n_manager)
        if language_selector.should_show_selector():
            selected_lang = language_selector.show_language_selection()
            i18n_manager.set_language(selected_lang, persist=True)
            logger.info(f"User selected language: {selected_lang}")

        # Show welcome screen
        show_welcome_screen(i18n_manager)

        # Main menu loop
        while True:
            try:
                action = show_main_menu(i18n_manager)
